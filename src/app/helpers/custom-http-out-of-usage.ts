...
export class CustomHttp extends Http {
    ...
    get(url: string, options?: RequestOptionsArgs): Observable<Response> {
        return super.get(environment.apiUrl+url, this.addJwt(options)).catch(this.handleError).map(res => res ? res.json() : {});
    }

    post(url: string, body: string, options?: RequestOptionsArgs): Observable<Response> {
        return super.post( environment.apiUrl+url, body, this.addJwt(options)).catch(this.handleError).map(res => res ? res.json() : {});
    }
